      ...
      <div className="flex-grow flex overflow-hidden">
        ...
        {activeTab === 'CHAT' && (
          <div className="flex-grow flex flex-col h-full">
            ...
            <form onSubmit={handleSendMessage} className="p-6 bg-slate-50 border-t border-slate-100 flex items-center space-x-4">
              <input 
                type="text" 
                placeholder="Type your message here..."
                className="flex-grow bg-white border border-slate-200 rounded-2xl px-6 py-4 focus:ring-2 focus:ring-primary outline-none transition-all shadow-inner"
                value={newMessage}
                onChange={(e) => setNewMessage(e.target.value)}
              />
              <button 
                disabled={sending || !newMessage.trim()}
                className="bg-primary text-white p-4 rounded-2xl hover:bg-primary-light transition-all shadow-lg active:scale-95 disabled:opacity-50"
              >
